            ...
            <div class="card shadow-sm">
              <?php
                ...
                if (!file_exists($imagen)) {
                  $imagen = "imagenes/no_foto.jpg";
                }
              ?>
              <img height="300px"  src="<?php echo $imagen; ?>" alt="">
              <div class="card-body">
                <h5 class="card-title corte"><?php echo $row['nombre']; ?></h5>
                <p class="card-text">$ <?php echo number_format($row['precio'], 2, '.', ','); ?></p>
                <div class="d-flex justify-content-between align-items-center">
                  <div class="btn-group">
                    <a href="details.php?id=<?php echo $row['id']; ?>&token=<?php echo hash_hmac('sha1', $row['id'], KEY_TOKEN); ?>" class="btn btn-negashop">Detalles</a>

                  </div>
                  <button type="button" class="btn btn-outline-negashop" onclick="addProducto(<?php echo $row['id']; ?>, '<?php echo hash_hmac('sha1', $row['id'], KEY_TOKEN); ?>')"><img class="minicarrito" src="img-logic/carrito.png" alt=""> Agregar</button>
                </div>
              </div>
            </div>
          </div>
